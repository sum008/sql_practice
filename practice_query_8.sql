...
-- select speciality, count(speciality) as speciality_count from (select ps.physician_id, speciality, patient_id, event_name from physician_speciality ps left join 
-- patient_treatment pt on ps.physician_id = pt.physician_id) sub_tbl_1 left join event_category ec 
-- on sub_tbl_1.event_name = ec.event_name where category = 'Procedure' group by speciality;


select * from (select ps.physician_id, speciality, patient_id, event_name from physician_speciality ps left join 
patient_treatment pt on ps.physician_id = pt.physician_id) sub_tbl_1 left join event_category ec 
on sub_tbl_1.event_name = ec.event_name where category = 'Procedure';
